  ...
  if (error || !quotation) return (
    <div className="min-h-screen bg-surface-950 flex items-center justify-center">
      <div className="glass-card p-8 text-center">
        <p className="text-red-400 text-lg">Quotation not found</p>
      </div>
    </div>
  );

  const client = quotation.clientId as any;
  const branch = quotation.branchId as any;
  ...
  return (
    <div className="min-h-screen bg-surface-950 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Quotation Card */}
        <div className="glass-card p-8 print:shadow-none print:border-none" id="quotation-content">
          {/* Header */}
          <div className="flex justify-between items-start border-b-2 border-primary-500 pb-6 mb-6">
            ...
            <div className="text-right">
              <h2 className="text-3xl font-bold text-primary-400 uppercase">Quotation</h2>
              <p className="text-sm text-surface-300 mt-1"><strong>Number:</strong> {quotation.quotationNumber}</p>
              <p className="text-sm text-surface-300"><strong>Date:</strong> {new Date(quotation.createdAt).toLocaleDateString()}</p>
              <p className="text-sm text-surface-300"><strong>Status:</strong> <span className={`font-semibold ${quotation.status === 'CONVERTED' ? 'text-green-400' : 'text-yellow-400'}`}>{quotation.status}</span></p>
            </div>
          </div>
          ...
          <div className="flex justify-between mb-8">
            <div className="bg-surface-800/30 p-4 rounded-xl w-[45%]">
              <span className="text-xs text-primary-400 font-medium uppercase tracking-wider">Quotation For:</span>
          ...
          <div className="flex flex-col items-end space-y-2 border-t border-surface-700 pt-6">
            <div className="flex justify-between w-64 text-sm">
              ...
              <span className="text-white">₹{quotation.subtotal}</span>
            </div>
            {quotation.tax > 0 && (
              <div className="flex justify-between w-64 text-sm">
                <span className="text-surface-400">Tax:</span>
                <span className="text-white">₹{quotation.tax}</span>
              </div>
            )}
            {quotation.discount > 0 && (
              <div className="flex justify-between w-64 text-sm">
                <span className="text-surface-400">Discount:</span>
                <span className="text-red-400">-₹{quotation.discount}</span>
              </div>
            )}
            <div className="flex justify-between w-64 text-lg font-bold border-t-2 border-primary-500 pt-2 mt-2">
              ...
              <span className="text-white">₹{quotation.total}</span>
            </div>
          </div>

          {/* Footer */}
          <div className="mt-12 text-center border-t border-surface-800 pt-6">
            <p className="text-sm text-white font-medium">This quotation is valid for 15 days.</p>
            <p className="text-[10px] text-surface-400 mt-2">
              Generated on {new Date().toLocaleString()} • This is a computer-generated quotation
            </p>
          </div>
        </div>
      </div>
